...
function renderTable(rows) {
    ...
    let html = "<table>";
    html += "<colgroup>";
    // Tylko jedna kolumna identyfikująca dzień.
    // Data z arkusza jest wyświetlana razem z nazwą dnia.
    ...
    dataRows.forEach((row) => {
        ...
        // Kolumna DATA została usunięta. Dzień i data są razem w jednej kolumnie.
        html += `<td class="day"><span class="day-name">${escapeHtml(fullDayName)}</span>${displayedDate ? `<span class="day-date">${escapeHtml(displayedDate)}</span>` : ""}</td>`;
